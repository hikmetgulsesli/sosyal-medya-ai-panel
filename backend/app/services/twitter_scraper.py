...
from scrapling.adaptors import TwitterAdaptor, Tweet
...
class TwitterScraperService:
    ...
    def _extract_profile_from_html(self, username: str, html: str) -> TwitterProfile:
        ...
        # Twitter embeds profile data in script tags as JSON
        ...
        if created_match:
            try:
                ...
                # Twitter date format: "Mon Nov 15 12:34:56 +0000 2021"
    ...
    def _extract_posts_from_html(self, html: str, max_posts: int = 20) -> List[TwitterPost]:
        ...
        # Look for tweet data in JSON format
        # Twitter embeds tweet data in the page
        tweet_pattern = r'"tweet":\{[^}]*"id":"(\d+)"[^}]*"text":"([^"]*)"[^}]*"user":\{[^}]*"screen_name":"([^"]*)"[^}]*"name":"([^"]*)"'
        
        # Alternative: Look for tweet objects in timeline data
        timeline_pattern = r'"id_str":"(\d+)".*?"full_text":"((?:[^"\\]|\\.)*)".*?"user":\{[^}]*"screen_name":"([^"]*)"[^}]*"name":"([^"]*)"'
        
        matches = list(re.finditer(timeline_pattern, html, re.DOTALL))
        
        for i, match in enumerate(matches[:max_posts]):
            try:
                ...
                like_match = re.search(rf'"favorite_count":(\d+)', html[match.start():match.start()+5000])
                reply_match = re.search(rf'"reply_count":(\d+)', html[match.start():match.start()+5000])
                retweet_match = re.search(rf'"retweet_count":(\d+)', html[match.start():match.start()+5000])
                ...
                post = TwitterPost(
                    ...
                    created_at=datetime.utcnow(),  # Will be updated if found
                ...
                timestamp_match = re.search(r'"created_at":"([^"]+)"', html[match.start():match.start()+5000])
                if timestamp_match:
                    try:
    ...
    def get_post_by_id(self, tweet_id: str) -> TwitterPost:
        ...
        try:
            ...
            # Extract tweet data using adaptor
            tweet = self.adaptor.extract_tweet_details(url, html)
            
            # Convert to TwitterPost
            post = TwitterPost(
                id=tweet_id,
                text=tweet.text or "",
